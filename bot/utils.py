...
def get_sheet_by_url(url_env_name: str):
    try:
        ...
        creds = ServiceAccountCredentials.from_json_keyfile_name(env.GOOGLE_CREDENTIALS_PATH, scope)
        client = gspread.authorize(creds)
        return client.open_by_url(url_env_name).sheet1
    except Exception as e:
        print(f"Unable to connect the table {url_env_name}: {e}")
        return None


CONTAINER_SHEET = get_sheet_by_url(env.CONTAINER_SHEET_URL)
CARGO_1_SHEET = get_sheet_by_url(env.CARGO_1_SHEET_URL)
CARGO_2_SHEET = get_sheet_by_url(env.CARGO_2_SHEET_URL)
...
def track(user_input: str, lang: str) -> dict:
    ...
    for row in rows:
        ...
        if user_input == container_id:
            ...
            reply_message = f"<b>{translate(lang, 'message.header')}</b>\n\n"
            for key, value in data.items():
                if not value:
                    continue
                label = translate(lang, f"message.full_container.{key}")
                reply_message += f"<b>{html.escape(label)}:</b> <code>{html.escape(value)}</code>\n"
            reply_message += f"<b>{translate(lang, 'message.date')}:</b> <code>{datetime.now().strftime('%d-%m-%Y')}</code>"
            reply_message += f"\n\n<b>{translate(lang, 'message.footer')}</b>"
...
def search_by_shipping_mark(user_input: str, lang: str) -> dict:
    ...
    for row in data:
        if str(row.get("Shipping mark", "")).strip().lower() == user_input:
            cargo_data = {
