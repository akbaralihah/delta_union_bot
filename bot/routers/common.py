...
from bot.reply import main_menu_buttons, admin_menu_buttons, language_keyboard
...
async def command_start_handler(msg: Message, session: AsyncSession) -> None:
    ...
    if not user:
        ...
        await msg.answer(
            translate("UZ", "choose_language"), reply_markup=language_keyboard("UZ")
        )
        return
    ...
    answer_text = translate(
        lang, "greeting", name=f"<b>{html.quote(msg.from_user.full_name)}</b>"
    )
    ...
    if msg.from_user.id in settings.ADMINS:
        ...
        await msg.answer(
            answer_text, reply_markup=main_menu_buttons(lang), parse_mode=ParseMode.HTML
        )


@router.message(
    F.text.in_([translate("UZ", "change_language"), translate("RU", "change_language")])
)
async def change_language(msg: Message, session: AsyncSession):
    lang = await User.get_user_lang(msg.from_user.id, session=session)
    await msg.answer(
        translate(lang, "choose_language"), reply_markup=language_keyboard(lang)
    )
...
async def set_language(call: CallbackQuery, session: AsyncSession):
    ...
    if call.from_user.id in settings.ADMINS:
        await call.message.answer(
            translate(lang, "main_menu"), reply_markup=admin_menu_buttons(lang)
        )
    else:
        await call.message.answer(
            translate(lang, "main_menu"), reply_markup=main_menu_buttons(lang)
        )
